...
@router.get(
    "/api/{workoutid}/exerciseinstances/",
    response_model=ListExerciseInstance
)
async def get_exercise_instance(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"An error has occurred: {str(e)}")


@router.get(
    "/api/{workoutid}/exerciseinstances/{exerciseinstanceid}/",
    response_model=ExerciseInstanceOut
)
...
@router.post(
    "/api/{workoutid}/exerciseinstance/",
    response_model=ExerciseInstanceOut
)
...
async def update_exercise_instance(
    ...
    try:
        updated_instance = queries.update_exercise_instance(
        ...
        raise HTTPException(
            status_code=500, detail=f"An error has occured: {str(e)}")
